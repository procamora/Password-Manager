...
def add_key(text):
	fn = tempfile.mktemp('pgpkey')
	fp = open(fn,'w')
	fp.write(text)
	stdin, stdout = os.popen4('pgpk -a '+fn)
	stdin.close()
	result = stdout.read()
	if result[0:6] == 'Unable':
...
# This adds the author's public key to the keychain on this computer if it does not
# already exist there.

if 'Matthew Schinckel <matt@null.net>' not in keys():
	add_key(pgp.author_key)
